...
trait custom_create {
	...
	public static function custom_create($args = []) {
		return \Injector::inst()->createWithArgs(static::custom_class_name() ?: get_called_class(), $args);
	}

	/**
	 * If not defined in exhibiting class just return the current class.
	 * @return string
	 */
	public static function custom_class_name() {
		return static::config()->get('custom_class_name');
	}
