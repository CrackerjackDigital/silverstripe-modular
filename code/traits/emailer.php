...
trait emailer {
	 ...
	 * @param string                   $subjectOrLangKey          subject for email or a language key to look subject
	 *                                                            up by
	 * @param string                   $messageOrBodyIfNoTemplate if no template matches then this will be the body, it
	 *                                                            will be provided as $Message in a found template
	 * @param string|array             $templatesOrBodyLangKey    name(s) of template to use (or list of alternatives),
	 *                                                            or a language key to lookup the body content with
	 *                                                            template body
	 * @param array                    $data
	 * @param null                     $error                     set to any error message etc if available
	 * @return bool
	 * @throws null
	 */
	public function emailer_send($senderAddressOrMemberID, $recipientAddressesOrMemberIDs, $subjectOrLangKey, $messageOrBodyIfNoTemplate, $templatesOrBodyLangKey = '', $data = [], &$error = null) {
		...
		$sources    = array_filter(
			[
				'provided'      => [
					$recipientAddressesOrMemberIDs,
					true,
				],
				'data'          => isset($directives['RECIPIENT']) ? [
					$directives['RECIPIENT'],
					true,
				] : [],
				'admin_email'   => [
		...
		if ($recipients && $senderAddress) {
			...
			$templatesOrBodyLangKey = array_filter(
				is_array($templatesOrBodyLangKey)
