...
abstract class Field extends ModelExtension {
	...
	// disable validation, e.g. during duplication or bulk upload, this can be set on the field or on the extended model to disable all validation
	...
	public function validationEnabled() {
		// enabled for model check first
		$enabled = $this()->config()->get( 'validation_enabled' );

		return is_null( $enabled )
			? \Config::inst()->get( get_class(), 'validation_enabled' )
			: $enabled;
	}
